...
class DebugRequestHandler(BaseHTTPRequestHandler):
    def __init__(self, response: Type, out: Path, *args, **kwargs) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--response", nargs="?", type=Type, default=Type.JSON)
    parser.add_argument("--out", nargs="?", type=Path, default="tmp")
    args = parser.parse_args()

    print(f"Start Dump HTTP Server at {args.host}:{args.port}")

    custom_handler = partial(DebugRequestHandler, args.response, args.out)
